...
Some demos can optionally capture, encode and/or decode live audio and video. When you pass `--enable-moq-examples` or `--enable-roq-examples`, the configure script will look for additional dependencies (FFmpeg, Opus, SDL2 and related libraries). If they're available, the build will also include the LOC publisher/subscriber demos (`imquic-moq-loc-send` and `imquic-moq-loc-recv`) and/or RoQ audio/video capture and display support in the sender and receiver respectively; otherwise a warning is printed and only the basic demos are built.
...
When using SVC codecs (e.g., `h264-svc` or `vp9-svc`), the receiver can limit which layers are decoded using `--svc-max-temporal-layer` and `--svc-max-spatial-layer`. By default, when no explicit temporal layer cap is set, the receiver will adaptively reduce the decoded SVC temporal layer in weak network conditions; this can be disabled via `--no-svc-adaptive`.
...
If the optional A/V dependencies are available, the RoQ sender can also capture audio/video locally instead of listening for RTP on UDP ports. This launches the sender as a client that captures from the default microphone and webcam, encodes Opus audio on flow ID `0` and H.264 video on flow ID `1`, and sends them to the receiver:
...
* `imquic-moq-pub`, a basic MoQ publisher (basically a clone of `moq-clock` in [moq-rs](https://github.com/kixelated/moq-rs));
* `imquic-moq-sub`, a basic MoQ subscriber (with support for a few different kinds of media);
* `imquic-moq-loc-send`, a MoQ publisher that can publish live audio/video using LOC (only built if the optional A/V dependencies are available);
* `imquic-moq-loc-recv`, a MoQ subscriber that can subscribe to (and render) live audio/video using LOC (only built if the optional A/V dependencies are available);
...
When relaying SVC-encoded LOC streams, the relay can drop objects above a given temporal or spatial layer via `--svc-max-temporal-layer` and `--svc-max-spatial-layer`. By default, when no explicit temporal layer cap is set, the relay will adaptively limit the forwarded SVC temporal layer per subscriber in weak network conditions; this can be disabled via `--no-svc-adaptive`.
...
Assuming a relay is available at that address and that a [moq-encoder-player](https://github.com/facebookexperimental/moq-encoder-player/) instance is publishing audio and video, this creates a MoQ subscriber (on WebTransport) to both tracks that prints the LOC header (`-t loc`) of each incoming object:
	...
	./examples/imquic-moq-sub -r 127.0.0.1 -R 4433 -w -H /moq -n vc -N 12345678-audio -N 12345678-video -A secret -t loc
...
The subscriber can also use `-u`/`--use-catalog` to fetch the published catalog and autodetect the audio and video track names to subscribe to. When using SVC codecs, the maximum decoded temporal and spatial layers can be capped via `-T`/`--svc-max-temporal-layer` and `--svc-max-spatial-layer`; by default, when no explicit temporal layer cap is set, the subscriber will adaptively reduce the decoded SVC temporal layer in weak network conditions (disable via `--no-svc-adaptive`).
