...
#define IMQUIC_ROQ_VP8_CLOCK 90000

static uint8_t imquic_roq_vp8_descriptor(gboolean first, gboolean keyframe) {
	uint8_t desc = 0;
	/* RFC 7741: N=1 marks a non-reference frame */
	if(!keyframe)
		desc |= 0x20;
	if(first)
		desc |= 0x10;
...
static size_t imquic_roq_vp8_descriptor_len(const uint8_t *payload, size_t payload_len) {
	size_t offset = 1;
	uint8_t ext = 0;
...
gboolean imquic_roq_rtp_packetize_vp8(imquic_roq_rtp_state *state,
	...
	if(state == NULL || frame == NULL || len == 0 || cb == NULL || fps <= 0)
		return FALSE;
	ts_increment = IMQUIC_ROQ_VP8_CLOCK / (uint32_t)fps;
	max_chunk = IMQUIC_ROQ_RTP_MAX_PACKET - 12 - 1;
	if(len <= max_chunk) {
		payload[0] = imquic_roq_vp8_descriptor(TRUE, keyframe);
	...
	while(offset < len) {
		...
		payload[0] = imquic_roq_vp8_descriptor(first, keyframe && first);
...
gboolean imquic_roq_rtp_depay_vp8(imquic_roq_vp8_depay *depay,
	...
	*frame_len = 0;
	...
	hdr_len = imquic_roq_vp8_descriptor_len(payload, payload_len);
	if(hdr_len == 0 || hdr_len >= payload_len)
		return FALSE;
	start = (payload[0] & 0x10) != 0;
	end = rtp_marker;
	if(depay->last_ts != timestamp) {
		imquic_roq_vp8_depay_reset(depay);
...
uint32_t imquic_roq_rtp_svc_spatial_ssrc(uint32_t base_ssrc, uint8_t spatial_id) {
	return (base_ssrc & ~IMQUIC_ROQ_SVC_SSRC_SPATIAL_MASK) | (uint32_t)spatial_id;
}
...
int imquic_roq_rtp_ssrc_spatial_id(uint32_t ssrc, int spatial_layers) {
	...
	if(spatial_layers <= 1)
		return -1;
	spatial = (uint8_t)(ssrc & IMQUIC_ROQ_SVC_SSRC_SPATIAL_MASK);
	if(spatial >= (uint8_t)spatial_layers)
		return -1;
	return (int)spatial;
}
...
size_t imquic_roq_rtp_build_svc_feedback(imquic_roq_rtp_state *state, uint8_t *buffer, size_t blen,
	...
	payload[4] = IMQUIC_ROQ_SVC_FEEDBACK_VERSION_LAYERS;
	payload[5] = max_temporal_layer;
	payload[6] = max_spatial_layer;
	payload[7] = 0;
...
gboolean imquic_roq_rtp_parse_svc_feedback(uint8_t *rtp, size_t rtp_len, uint8_t *max_temporal_layer,
	...
	magic = ntohl(magic);
	if(magic != IMQUIC_ROQ_SVC_FEEDBACK_MAGIC ||
			(payload[4] != IMQUIC_ROQ_SVC_FEEDBACK_VERSION_TEMPORAL &&
			 payload[4] != IMQUIC_ROQ_SVC_FEEDBACK_VERSION_LAYERS))
		return FALSE;
	*max_temporal_layer = payload[5];
	if(max_spatial_layer != NULL) {
		if(payload[4] >= IMQUIC_ROQ_SVC_FEEDBACK_VERSION_LAYERS)
			*max_spatial_layer = payload[6];
		else
			*max_spatial_layer = 0xFF;
	}
	return TRUE;
}
