...
static gboolean video_decode_synced = TRUE;
...
static gboolean imquic_demo_should_decode_video(uint64_t group_id, uint64_t object_id, gboolean keyframe) {
	if(video_decode_synced)
		return TRUE;
	if(!keyframe) {
		IMQUIC_LOG(IMQUIC_LOG_VERB, "Waiting for keyframe after loss, skipping object %"SCNu64"/%"SCNu64"\n",
			group_id, object_id);
		return FALSE;
	}
	return TRUE;
...
static void imquic_demo_new_connection(imquic_connection *conn, void *user_data) {
	...
	if(svc_adaptive_enabled || (moq_loc_svc_is_svc_codec(codec) && !options.no_svc_adaptive && svc_max_temporal_cap < 0))
...
int main(int argc, char *argv[]) {
	...
	if(svc_max_temporal_cap >= 0) {
		...
		IMQUIC_LOG(IMQUIC_LOG_INFO, "SVC adaptive layer selection will activate when an SVC track is detected\n");
	}
