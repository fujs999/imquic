...
gboolean demo_options_parse(demo_options *options, int argc, char *argv[]) {
	...
	GOptionEntry opt_entries[] = {
		...
		{ "svc-max-temporal-layer", 'T', 0, G_OPTION_ARG_INT, &options->svc_max_temporal_layer, "Drop relayed SVC objects above this temporal layer (-1=adaptive in weak networks, default=-1)", "0-3" },
		...
		{ "no-svc-adaptive", 0, 0, G_OPTION_ARG_NONE, &options->no_svc_adaptive, "Disable per-subscriber SVC layer adaptation on the relay (default=adaptive when --svc-max-temporal-layer is not set)", NULL },
