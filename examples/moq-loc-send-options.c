...
gboolean demo_options_parse(demo_options *options, int argc, char *argv[]) {
	...
	GOptionEntry opt_entries[] = {
		...
		{ "video-bitrate", 'B', 0, G_OPTION_ARG_INT, &options->video_bitrate, "Video encoding bitrate, in bits per second (default=1048576)", "bps" },
		{ "no-adaptive", 'Z', 0, G_OPTION_ARG_NONE, &options->no_adaptive, "Disable adaptive bitrate/framerate/resolution control (default=adaptive enabled)", NULL },
		{ "no-adaptive-resolution", 0, 0, G_OPTION_ARG_NONE, &options->no_adaptive_resolution, "Disable adaptive resolution only (default=enabled when adaptive is on)", NULL },
		{ "no-adaptive-bitrate", 0, 0, G_OPTION_ARG_NONE, &options->no_adaptive_bitrate, "Disable adaptive video bitrate only (default=enabled when adaptive is on)", NULL },
		{ "no-adaptive-framerate", 0, 0, G_OPTION_ARG_NONE, &options->no_adaptive_framerate, "Disable adaptive framerate only (default=enabled when adaptive is on)", NULL },
		{ "video-codec", 'e', 0, G_OPTION_ARG_STRING, &options->video_codec, "Video codec to use (default=h264-avcc)", "h264-avcc|h264-annexb|h264-svc|vp8|vp9|vp9-svc|av1" },
		{ "svc-temporal-layers", 'T', 0, G_OPTION_ARG_INT, &options->svc_temporal_layers, "Number of SVC temporal layers for h264-svc/vp9-svc (default=2, max=4)", "2-4" },
